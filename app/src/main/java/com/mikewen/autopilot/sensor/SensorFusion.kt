...
class SensorFusion {
    ...
    private val MAG_CAL_RMC_SPEED_MS  = 1.5f           // m/s (≈ 2.9 kt)
    private val MAG_CAL_TAR_ACC_MAX    = 5.0f           // degrees; quality=4 typically 0.1–1°
    private val MAG_CAL_SEA_STATE      = 0.3f           // relaxed: PQTMTAR is robust to motion
    private val MAG_CAL_WINDOW         = 60             // more samples → more stable
     ...
     * Called from processA2 with PQTMTAR as primary reference.
     *
     * @param tarHeadingDeg   PQTMTAR corrected heading (post misalignment)
     * @param tarAccDeg       PQTMTAR accuracy in degrees (lower = better)
     * @param gnssQuality     4 = RTK fixed (trusted at any speed), 6 = DR, else skip
    ...
    private fun updateMagCalibration(
        ...
        // Only accept RTK-fixed PQTMTAR with good accuracy — no speed requirement
        if (gnssQuality != 4 || tarAccDeg >= MAG_CAL_TAR_ACC_MAX) return
        val tarWeight = (1f - tarAccDeg / MAG_CAL_TAR_ACC_MAX).coerceIn(0.1f, 1.0f)

        // Blend RMC COG only when speed > 1.5 m/s and accuracy is high
        val speedMs = cachedRmcSpeed / 1.94384f
        val rmcWeight = if (cachedRmcValid && speedMs >= MAG_CAL_RMC_SPEED_MS && tarAccDeg < 2.0f)
        ...
        if (rmcWeight > 0f) {
            ...
            while (diff < -180f) diff += 360f
            refHeading = ((tarHeadingDeg + (rmcWeight / (tarWeight + rmcWeight)) * diff) + 360f) % 360f
        } else {
        ...
        // High-quality samples written twice to converge faster
    ...
    fun processA1(
        ...
        val magConf: Float = if (gnssRecent) {
            ...
            // Tilt penalty only: tilted mag is unreliable
    ...
    fun processA2(
        ...
        // Auto-calibrate mag: PQTMTAR quality=4 is trusted at any speed;
        // RMC COG blended in only when speed > 1.5 m/s and accuracy high.
    ...
    fun processNmeaRmc(
        ...
        // cogDeg: GpsManager always passes null — phone GPS provides speed+position only,
        // never heading. The 0.03f weight is a safety net only.
        ...
        // position is needed for declination, map centering, and bearing calculation.
        state = state.copy(
            headingDeg        = heading,
            speedKnots        = speedKt,
